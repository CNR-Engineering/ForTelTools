...
!
      IF(ABS(XDEP-XARR)+ABS(YDEP-YARR).LT.EPS) THEN
        ...
        WRITE(*,*) ' '
        IF(LNG.EQ.1) THEN
          WRITE(*,*) 'INPOLY : POINTS CONFONDUS DANS LE POLYGONE'
          WRITE(*,*) 'AU POINT DE COORDONNEES : ',XDEP,'  ET  ',YDEP
          WRITE(*,*) 'DE NUMERO ',N
          IF(N.EQ.NSOM) THEN
            WRITE(*,*) 'LE DERNIER POINT NE DOIT PAS ETRE EGAL AU'
            WRITE(*,*) 'PREMIER (POUR UN TRIANGLE, PAR EXEMPLE,'
            WRITE(*,*) 'IL FAUT DONNER TROIS POINTS ET NON QUATRE)'
          ENDIF
          WRITE(*,*) 'INPOLY EST PROBABLEMENT CALLED BY FILPOL'
        ENDIF
        IF(LNG.EQ.2) THEN
          WRITE(*,*) 'INPOLY: SUPERIMPOSED POINTS IN THE POLYGON'
          WRITE(*,*) 'AT POINT: ',XDEP,'  AND  ',YDEP,' WITH NUMBER ',N
          IF(N.EQ.NSOM) THEN
            WRITE(*,*) 'THE LAST POINT MUST NOT BE EQUAL TO THE FIRST'
            WRITE(*,*) 'FOR EXAMPLE, GIVE 3 POINTS FOR A TRIANGLE'
          ENDIF
          WRITE(*,*) 'INPOLY IS PROBABLY CALLED BY FILPOL'
        ENDIF
        WRITE(*,*) ' '
        WRITE(*,*) ' '
        STOP
